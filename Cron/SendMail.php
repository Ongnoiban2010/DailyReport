...
class SendMail
{
	...
	public function export()
	{
        ...
        if($enable == 1) {
        	...
        	$mail->setFrom($username, 'Admin')->addTo($receiveEmail)->setSubject('Notification')->setBodyHtml($htmlBody);
    ...
    public function getProduct()
    {
    	...
    	echo $to2;
  //   	$to3 = $this->_date->date(new \DateTime($to2))->format('Y/m/d H:i:s');
  //   	echo $to3;
  ...
  //   	$obj = \Magento\Framework\App\ObjectManager::getInstance();
    	...
    	$from = strtotime('-1 day', strtotime($to2));
	        $from = date('Y-m-d h:i:s', $from); // 1 days before
	        

	        $orders = $this->itemFactory->create()->getCollection()->addFieldToFilter('created_at', array('from'=>$from, 'to'=>$to2));
	        $data = [];
	        if (count($orders) > 0) {
